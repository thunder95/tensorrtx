...
static const int INPUT_DIM = segnum * seglen * target_size * target_size; //输入变量的维度: 7*15*224*224
std::string INPUT_BLOB_NAME = "image"; //输入节点的名称
std::string OUTPUT_BLOB_NAME = "save_infer_model/scale_0"; //输出节点的名称
std::string wts_file = "../../model_in/pd_stnet.wts"; //原权重文件
std::string engine_file =  "../../model_in/pd_stnet_10.trtengine"; //生成的trt引擎文件
...
ICudaEngine* createEngine(IBuilder* builder, DataType dt)
{
    ...
    ITensor* data = network->addInput(INPUT_BLOB_NAME.c_str(), dt, Dims4{segnum, seglen, target_size, target_size});
    assert(data);
    ...
    IPoolingLayer* pool3 = network->addPooling(*x, PoolingType::kMAX, DimsHW{segnum, 1});
    ...
    assert(fc1);
    //dims1 = fc1->getOutput(0)->getDimensions();


    //softmax
    ISoftMaxLayer* prob = network->addSoftMax(*fc1->getOutput(0));
    assert(prob);
    //dims1 = prob->getOutput(0)->getDimensions();
    ...
    std::cout<<"cores: "<<builder->getNbDLACores()<<std::endl;
    if (useDLACore>0) {
...
void test_infer(ICudaEngine& engine, IExecutionContext& context,  std::unique_ptr<float>& data, std::unique_ptr<float>& prob) {
    ...
    std::cout<<"input idx: "<<inputIndex<<std::endl;
    std::cout<<"output idx: "<<outputIndex<<std::endl;
    std::cout<<"input dims: "<<batchSize * INPUT_DIM * sizeof(float)<<std::endl;
